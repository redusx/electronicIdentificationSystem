 ...
 */
package com.example.countercamtest.objectdetection.objectdetector
...
class ObjectDetectorHelper(
    ...
    fun setupObjectDetector() {
        ...
        try {
            Log.i(TAG, "🔧 Setting up ObjectDetector with model=$modelName, threshold=$threshold, maxResults=$maxResults")
            val optionsBuilder =
            ...
            Log.i(TAG, "✅ ObjectDetector created successfully with model: $modelName")
    ...
    fun detectImage(image: Bitmap): ResultBundle? {
        ...
        if (objectDetector == null) {
            Log.e(TAG, "❌ ObjectDetector is null - cannot run detection")
            return null
        }

        Log.d(TAG, "🖼️ Running detection on ${image.width}x${image.height} image")
        ...
        objectDetector?.detect(mpImage)?.also { detectionResult ->
            ...
            val detectionsCount = detectionResult.detections()?.size ?: 0

            Log.i(TAG, "🎯 Detection completed in ${inferenceTimeMs}ms, found $detectionsCount objects with threshold=$threshold")

            // Log all detections with their scores
            if (detectionsCount > 0) {
                detectionResult.detections()?.forEachIndexed { index, detection ->
                    ...
                    if (categories.isNotEmpty()) {
                        ...
                        Log.i(TAG, "🔸 Detection #$index: ${topCategory.categoryName()} (score: ${topCategory.score()}, bbox=[${bbox.left}, ${bbox.top}, ${bbox.right}, ${bbox.bottom}])")
                    }
                }
            } else {
                Log.w(TAG, "⚠️ No objects detected with threshold=$threshold. Try lowering threshold or point camera at common objects (person, phone, book, etc.)")
            }
        ...
        // to indicate this.
        Log.e(TAG, "❌ objectDetector.detect() returned null")
        return null
    }
